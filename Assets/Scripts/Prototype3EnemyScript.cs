...
public class Prototype3EnemyScript : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		else if(isAttacking)
		{
			...
			if(transform.position.y <= enemyYPostion-attackDistance && sizeChangeSpeed >0)
			{
				...
				Prototype3Layout.AttackPlayer(10);//remove 10 health from player
			}
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		...
		{
			Prototype3FireBallScript script = other.gameObject.GetComponent<Prototype3FireBallScript>();
		...
		{
			Prototype3ArrowScript script = other.gameObject.GetComponent<Prototype3ArrowScript>();
		...
		{
			Prototype3SwordScript script = other.gameObject.GetComponent<Prototype3SwordScript>();
			if(script.getIsThrown() || script.getIsTouched())
			{
				health -= 20;
				StartCoroutine(Flash(collideColor,normalColor));
