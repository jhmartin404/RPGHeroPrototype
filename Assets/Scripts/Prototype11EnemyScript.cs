...
public class Prototype11EnemyScript : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		else if(isAttacking)
		{
			...
			if(transform.position.y <= enemyYPostion-attackDistance && sizeChangeSpeed >0)
			{
				...
				Prototype11Layout.AttackPlayer(damage);//remove 10 health from player
			}
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		...
		{
			Prototype11FireBallScript script = other.gameObject.GetComponent<Prototype11FireBallScript>();
		...
		{
			Prototype11ArrowScript script = other.gameObject.GetComponent<Prototype11ArrowScript>();
