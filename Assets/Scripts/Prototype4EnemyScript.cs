...
public class Prototype4EnemyScript : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		else if(isAttacking)
		{
			...
			if(transform.position.y <= enemyYPostion-attackDistance && sizeChangeSpeed >0)
			{
				...
				Prototype4Layout.AttackPlayer(10);//remove 10 health from player
			}
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		...
		{
			Prototype4FireBallScript script = other.gameObject.GetComponent<Prototype4FireBallScript>();
		...
		{
			Prototype4ArrowScript script = other.gameObject.GetComponent<Prototype4ArrowScript>();
		...
		{
			Prototype4SwordScript script = other.gameObject.GetComponent<Prototype4SwordScript>();
			if(script.getIsThrown() || script.getIsTouched())
			{
				health -= 20;
				Destroy(other.gameObject);
